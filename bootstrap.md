# Project Bootstrap — Multi-Agent SDLC Framework

> **Purpose**: One-shot bootstrapping guide to initialize any project with a 17-agent team, 8-round sprint pipeline, 8 mandatory quality gates, and full document structure.
>
...
- **Product**: [YOUR_PROJECT_NAME] — [One sentence description of what the product does]
- **Project directory**: `[YOUR_PROJECT_ROOT]`
...
[YOUR_PROJECT_NAME] is [one sentence description]. Tech stack: [PLACEHOLDER — e.g., Next.js, React, TypeScript, Tailwind CSS, your database, etc.].
...
| 6 | [backend-engineer] | R3 | Yes | [PLACEHOLDER — backend code paths, e.g., `src/app/api/`, `db/migrations/`] |
| 7 | [ai-ml-engineer] | R3 | Yes | [PLACEHOLDER — AI/ML code paths, e.g., `src/lib/ai/`] |
| 8 | [frontend-engineer] | R4 | Yes | [PLACEHOLDER — frontend code paths, e.g., `src/components/`, `src/app/`] |
...
| 13 | [test-engineer] | R6 | Yes | [PLACEHOLDER — test paths, e.g., `e2e/`, `src/__tests__/`] |
...
| [qa-reviewer] | `model: "sonnet"` | Checklist-driven (type safety, error handling, accessibility). Three other R5 reviewers overlap. |
...
Small items — bug fixes, minor UI changes, copy updates, single-endpoint additions.
...
- [qa-reviewer]: Type safety, error handling, patterns, edge cases, accessibility
...
### Privacy & Compliance
- [YOUR_COMPLIANCE_REQUIREMENTS — Define your privacy/compliance requirements, e.g., GDPR, CCPA, regional laws, etc.]
...
- [PLACEHOLDER — Define localization requirements: languages, date formats, regional conventions, etc.]
...
*Sprint 0 — Project Definition. First sprint for [YOUR_PROJECT_NAME].*
...
```markdown
# [YOUR_PROJECT_NAME] — Product Backlog
...
```markdown
# [YOUR_PROJECT_NAME] — Product Board
...
```markdown
# [YOUR_PROJECT_NAME] — Product Requirements Document (PRD)
...
[How [YOUR_PROJECT_NAME] solves each pain point]

### 1.5 Competitive Positioning
[Market map — competitors and [YOUR_PROJECT_NAME]'s unique position]
...
```markdown
# [YOUR_PROJECT_NAME] — Solution Design
...
[PLACEHOLDER — tables, relationships, access control policies]
...
```markdown
# [YOUR_PROJECT_NAME] — Business Plan
...
## 3. Solution
[How [YOUR_PROJECT_NAME] solves the problem]
...
```markdown
# [YOUR_PROJECT_NAME] — Marketing Plan
...
```markdown
# [YOUR_PROJECT_NAME] — Strategic Plan
...
[Where [YOUR_PROJECT_NAME] fits vs competitors]
...
```markdown
# [YOUR_PROJECT_NAME] — Release Notes
...
Follow these steps to initialize your project with the 17-agent framework:

### Step 0 — Prerequisites
Before starting:
- [ ] Claude Code installed and configured
- [ ] A GitHub repository created for your project
- [ ] Your project's tech stack decided
...
Copy the CLAUDE.md template from Section 2 into `[YOUR_PROJECT_ROOT]/CLAUDE.md`. Then customize:
- [ ] Fill in the **Project Overview** with your product name and tech stack
- [ ] Fill in the **Agent Roster** file ownership paths (rows 6-8, 13)
- [ ] Fill in the **Security** rules (data isolation, access control)
...
```bash
mkdir -p [YOUR_PROJECT_ROOT]/agents
```
...
```bash
mkdir -p [YOUR_PROJECT_ROOT]/docs/user-manual
```

### Step 6: Initial commit
```bash
/usr/bin/git add -A
/usr/bin/git commit -m "chore: bootstrap [YOUR_PROJECT_NAME] with agent pipeline and document structure"
```
